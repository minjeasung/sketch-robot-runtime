...
- ROS/MoveIt TCP frame 기준 → 롤러 중심까지 TCP local -Y 방향 거리:
  0.256700 m (= AFT200 0.0522 m + roller STEP forward 0.204500 m)
- 롤러 직경: 0.052 m (Φ52mm)
- 롤러 길이 (축 방향): 0.22 m
- Mount axis: 코드 변수 TOOL_AXIS="-Y"/"-y" 로 관리
- 실제 base 보정 후 스케치 목표 자세에서는 TCP local -Y 가 base +X,
  즉 벽 접근 방향을 향한다.
- 작업 시 EE 와 벽 거리: 0.256700 + 0.026 = 0.282700 m (벽 표면 ~ TCP)
...
- 페인트 롤러 attached collision (cylinder: r=0.026, L=0.22)
