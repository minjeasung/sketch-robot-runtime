...
class TestableAdmittanceController : public admittance_controller::AdmittanceController
{
  ...
  FRIEND_TEST(AdmittanceControllerTest, damping_absolute_overrides_ratio_per_axis);
  FRIEND_TEST(AdmittanceControllerTest, damping_defaults_to_the_ratio_derivation);
...
public:
  ...
  void seed_fresh_force_inputs_for_test()
  {
    const int64_t now_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(
                             std::chrono::steady_clock::now().time_since_epoch())
                             .count();
    admittance_controller::TimedWrenchCommand wrench;
    wrench.message.wrench.force.y = 1.0;
    wrench.received_steady_ns = now_ns;
    input_wrench_command_.set(wrench);

    admittance_controller::TimedComplianceCommand enable;
    enable.enabled = true;
    enable.received_steady_ns = now_ns;
    input_compliance_enable_.set(enable);
  }

  bool compliance_active_for_test() const { return compliance_active_; }

  geometry_msgs::msg::Wrench read_wrench_from_hardware_for_test()
  {
    geometry_msgs::msg::Wrench wrench;
    read_state_from_hardware(joint_state_, wrench);
    return wrench;
  }
