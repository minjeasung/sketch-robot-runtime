...
class Robot {
 public:
  ...
  /// Return the cached state together with transport and freshness metadata.
  /// A successful subsequent SDK sample restores validity automatically.
  StateSample read_once_with_status(double max_age_s = 0.1);
  ...
  // Free-drive (gravity-compensation / direct-teaching) On/Off.
  // While On the arm is hand-guidable; the caller MUST stop sending servo
  // commands (move_servo_j) for it to take effect.
  bool set_freedrive_mode(bool on);
 ...
 private:
  ...
  std::mutex lock_;
  bool updated_{false};
  bool state_transport_valid_{false};
