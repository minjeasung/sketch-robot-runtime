...
Robot::Robot(const string& ip, bool simulation, const rclcpp::Logger& logger)
  ...
  read_thread_ = thread([this]() {
    static rclcpp::Clock data_error_clock(RCL_STEADY_TIME);
    ...
    while (!read_thread_stop_.load()) {
      ...
      try {
        ...
        if (!s.has_value()) {
          {
            ...
            state_transport_valid_ = false;
          }
          RCLCPP_ERROR_THROTTLE(getLogger(), data_error_clock, 2000,
                                "request_data() returned no robot state; cached telemetry is invalid");
          continue;
        }
        {
          scoped_lock ul(read_lock_);
        ...
        RCLCPP_ERROR_THROTTLE(getLogger(), data_error_clock, 2000,
                              "request_data() failed; cached telemetry is invalid: %s", ex.what());
      } catch (...) {
        {
          scoped_lock ul(read_lock_);
          state_transport_valid_ = false;
        ...
        RCLCPP_ERROR_THROTTLE(getLogger(), data_error_clock, 2000,
                              "request_data() failed with an unknown exception; cached telemetry is invalid");
      }
    }
  });
  {
...
bool Robot::set_freedrive_mode(bool on) {
  scoped_lock sl(lock_);
  auto ret = cobot_.set_freedrive_mode(rc_, on, 2.0 /*timeout s*/);
  rc_error_check();
  return ret.is_success();
}
...
rb::podo::SystemState Robot::read_once() {
  const auto sample = read_once_with_status(0.1);
  RCLCPP_WARN_EXPRESSION(getLogger(), !sample.fresh,
                         "state sample is invalid or was updated %lf seconds ago (it may cause a problem)",
                         sample.age_s);
  return sample.state;
}

Robot::StateSample Robot::read_once_with_status(double max_age_s) {
  ...
  StateSample sample;
  sample.state = state_;
  sample.age_s = elapsed_in_sec;
  sample.transport_valid = state_transport_valid_;
  sample.fresh = state_sample_is_fresh(state_transport_valid_, elapsed_in_sec, max_age_s);
  return sample;
}
