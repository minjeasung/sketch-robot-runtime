...
RobotNode::RobotNode(const rclcpp::NodeOptions& options, shared_ptr<Robot> robot)
  ...
  system_state_publisher_ = this->create_publisher<rbpodo_msgs::msg::SystemState>("~/system_state", 10);
  //
  run_ = true;
...
void RobotNode::publish_system_state(const podo::SystemState& state) {
  if (system_state_publisher_) {
    system_state_publisher_->publish(convert_to_ros_msg(state));
  }
}
...
rbpodo_msgs::msg::SystemState RobotNode::convert_to_ros_msg(const podo::SystemState& state) {
  ...
  msg.op_stat_collision_status_raw = state.sdata.op_stat_collision_occur;
  msg.op_stat_sos_flag = state.sdata.op_stat_sos_flag;
  msg.op_stat_self_collision = (state.sdata.op_stat_self_collision == 1);
  msg.op_stat_self_collision_status_raw = state.sdata.op_stat_self_collision;
