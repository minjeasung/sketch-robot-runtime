...
painting_config_file_cfg = LaunchConfiguration("painting_config_file")
launch_rviz_cfg = LaunchConfiguration("launch_rviz")
...
def generate_launch_description():
    declared_arguments = [
        ...
        DeclareLaunchArgument("use_admittance", default_value="false"),
        DeclareLaunchArgument("launch_rviz", default_value="true"),
        DeclareLaunchArgument("admittance_profile", default_value="default"),
        DeclareLaunchArgument("admittance_use_case", default_value="free_push"),
        DeclareLaunchArgument(
            "painting_config_file",
            default_value="",
            description=(
                "Optional shared painting YAML appended last to ros2_control "
                "parameters; the top-level painting launch supplies it"
            ),
        ),
...
def _validate_painting_config_contract(
    *, use_admittance, admittance_use_case, painting_config_file
):
    """Require the runtime-tare/controller profile for painting admittance."""

    if (
        bool(use_admittance)
        and str(admittance_use_case).strip().lower() == "painting"
        and not str(painting_config_file).strip()
    ):
        raise RuntimeError(
            "admittance_use_case=painting requires painting_config_file; "
            "the legacy profile does not configure execution-scoped F/T tare"
        )
...
def launch_setup(context, *args, **kwargs):
    ...
    painting_config_file = context.perform_substitution(
        painting_config_file_cfg
    ).strip()
    if painting_config_file:
        painting_config_file = os.path.abspath(
            os.path.expanduser(painting_config_file)
        )
        if not os.path.isfile(painting_config_file):
            raise RuntimeError(
                f"painting_config_file does not exist: {painting_config_file}"
            )
    if admittance_use_case not in ("free_push", "painting"):
        raise RuntimeError("admittance_use_case must be 'free_push' or 'painting'")
    _validate_painting_config_contract(
        use_admittance=use_admittance,
        admittance_use_case=admittance_use_case,
    ...
    if use_admittance:
        ...
        if painting_config_file:
            # The shared system file is authoritative for the top-level
            # painting launch and therefore must be the final override.
            ros2_control_parameters.append(painting_config_file)
